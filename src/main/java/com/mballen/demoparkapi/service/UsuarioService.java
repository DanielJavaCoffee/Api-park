...
public class UsuarioService {
    ...
    @Transactional
    public ResponseEntity<UsuarioPatchSenhaDto> atualizarSenha(UsuarioPatchSenhaDto usuarioPatchSenhaDto) {
        ...
        if (usuarioOptional.isPresent()){
            ...
            usuarioRepository.save(usuario);
            return ResponseEntity.status(HttpStatus.OK).body(usuarioPatchSenhaDto);
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
    }
